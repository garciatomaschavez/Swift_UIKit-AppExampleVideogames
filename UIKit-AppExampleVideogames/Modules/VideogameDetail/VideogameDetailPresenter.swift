...
class VideogameDetailPresenter: VideogameDetailPresenterProtocol, VideogameDetailInteractorOutputProtocol {
    ...
    private let videogameId: String // Business key (e.g., title)
    ...
    func didTapFavoriteButton() {
        ...
        view?.showLoading()
        interactor?.toggleFavoriteStatus(forId: entity.id) // Use entity.id (business key)
    }
    ...
    func didUpdateFavoriteStatus(forVideogame videogame: VideogameEntity) {
        self.currentVideogameEntity = videogame // Update the stored entity
        view?.hideLoading()
        let viewModel = mapEntityToDetailViewModel(videogame)
        view?.displayVideogameDetails(viewModel) // Refresh the view with new details
    }
    ...
    private func mapEntityToDetailViewModel(_ entity: VideogameEntity) -> VideogameDetailViewModel {
        // VideogameDetailViewModel.id is String?
        // VideogameEntity.uuid is String? and currently always nil.
        let viewModelId = entity.uuid // This will be nil.
